...
```{bash run api, eval=T, echo=F, message=F, include=F}
...
pluck(df_list, "eligibility") %<>%
  mutate(screened = if_else(!duplicated(childID) | is.na(childID), 1, 0),
         eligible = if_else(str_detect(inclusion_criteria, el_crit) & 
                              (muac_under115==1 | whz_zscore_ind==1) & !duplicated(childID), 1, 0),
         refused = if_else(eligible==1 & is.na(childID), 1, 0),
         enrolled = if_else(!is.na(childID) & !duplicated(childID), 1, 0))
...
ggplot(total_perday, aes(x = interview_date, y = as.integer(total), color = Step)) +  
  ...
  scale_x_date(labels = date_format("%m/%d"), breaks = date_breaks("2 week")) +
...
pluck(df_list, "eligibility") %<>%
  mutate(exclz_abx_use = if_else(str_detect(inclusion_criteria, "abx_use"), 0, 1),
         ...
         exclz_growth_faltering_illness = if_else(str_detect(inclusion_criteria,
                                                             "growth_faltering_illness"), 0, 1),
...
df_reasons %>%
  select(csps_fra, exclz_prior__admission, exclz_feeding, exclz_Study_area, 
         exclz_available, exclz_consent) %>%
...
### WHZ ridgeline plots
```{r, echo=F, warning=F, fig.height = 4.35, fig.width = 6.5}
# plot whz ridges by time point
pluck(df_list, "anthro") %>%
  ggplot(aes(whz_zscore, time_point, fill=factor(stat(quantile)))) +
  stat_density_ridges(
...
WHZ densities at each time point, colored by quartile of the distribution.

```{r, echo=F, warning=F, fig.height = 4.35, fig.width = 6.5}
pluck(df_list, "anthro") %>%
  ggplot(aes(whz_zscore, time_point)) +
  geom_density_ridges(jittered_points=T, alpha=.7) +
  xlab("WHZ")
```

WHZ densities at each time point, with the actual data points (jittered) filling each distribution.

### MUAC ridgeline plots
```{r, echo=F, warning=F, fig.height = 4.35, fig.width = 6.5}
# plot ridges of MUAC
pluck(df_list, "anthro") %>%
  ggplot(aes(muac, time_point, fill=factor(stat(quantile)))) +
  stat_density_ridges(
    geom = "density_ridges_gradient", calc_ecdf = TRUE,
    quantiles = 4, quantile_lines = TRUE
  ) +
...
MUAC densities at each time point, colored by quartile of the distribution.

```{r, echo=F, warning=F, fig.height = 4.35, fig.width = 6.5}
pluck(df_list, "anthro") %>%
  ggplot(aes(muac, time_point)) +
  geom_density_ridges(jittered_points=T, alpha=.7)
```

MUAC densities at each time point, with the actual data points (jittered) filling each distribution.
...
temp <- inner_join(
  ...
  pluck(df_list, "anthro") %>%
    ...
    mutate_at(vars(-matches("childID"), -matches("baseline")), 
              list(dif = ~ ((. - baseline)*1000)/baseline)) %>%
  ...
  mutate(week_1_vel=week_1_dif/week_1_days,
         ...
         week_5_vel=week_5_dif/week_5_days,
         week_6_vel=week_6_dif/week_6_days,
         week_7_vel=week_7_dif/week_7_days,
         week_8_vel=week_8_dif/week_8_days)
...
bind_rows(
  ...
  summarise(week_1=mean_sd(week_1_vel),
            ...
            week_5=mean_sd(week_5_vel),
            week_6=mean_sd(week_6_vel),
            week_7=mean_sd(week_7_vel),
            week_8=mean_sd(week_8_vel)),
  temp %>%
    summarise(week_1=mean_sd(week_1_vel),
              week_2=mean_sd(week_2_vel),
              week_3=mean_sd(week_3_vel),
...
## Reason not treated (Display `r DN`)
```{r, echo=F, message=F, warning=F}
# ids enrolled and treated
ids_en <- pluck(df_list, "eligibility") %>% filter(!is.na(childID)) %>% distinct(childID) %>% pull
ids_tr <- pluck(df_list, "treatment") %>% filter(!is.na(childID)) %>% distinct(childID) %>% pull

# subset and table
pluck(df_list, "treatment") %>% 
  filter(childID %in% setdiff(ids_en, ids_tr)) %>%
  select(id, csps_fra, child_firstname, childID, no_admin_treatment) %>%
  mutate(no_admin_treatment = ifelse(is.na(no_admin_treatment), "None", no_admin_treatment)) %>%
  kable("latex", col.names = c("Interview", "CSPS", "Child name", "Child ID", "Reason untreated")) %>%
  row_spec(c(0), bold=T)
```
...
bind_rows(
  ...
  kable("latex", col.names = c("CSPS", "Total", "Positives", "Negatives",
...
```{r, echo=F, message=F, warning=F}
# number of csps
overall_rowno <- pluck(df_list, "malaria") %>% 
  filter(time_point=="week_8") %>%
...
Participants randomly selected for rectal swab by CSPS

## Week 8 swabs
```{r, echo=F, message=F, warning=F}
# number of csps
overall_rowno <- pluck(df_list, "rectal") %>% filter(timepoint=="week_8") %>%
  distinct(csps_fra) %>% nrow + 1

# tabulate number of rectal swabs
bind_rows(
  pluck(df_list, "rectal") %>%
    filter(timepoint=="week_8") %>%
    group_by(CSPS=csps_fra) %>%
    summarise(selected=sum(rectal_alloc),
              performed=sum(performed, na.rm = T)),
  pluck(df_list, "rectal") %>%
...
df_dis <- pluck(df_list, "discharge") %>%
  left_join(., 
            pluck(df_list, "eligibility") %>% 
              select(childID, enroll_date=start_time),
            by="childID") %>%
  mutate(time_point=round(as.numeric(difftime(as.Date(start_time), enroll_date, units = "weeks"))),
...
df_5to7 <- pluck(df_list, "eligibility") %>%
  ...
  left_join(., 
            pluck(df_list, "followup") %>% 
              filter(!duplicated(uniqID)) %>%
              select(id, childID, time_point, vital_status), 
            by=c("childID", "time_point")) %>%
  select(childID, enroll_date, time_point, expected, id, csps_fra, child_firstname, vital_status)
...
df_followup %>%
  mutate(actual=ifelse(vital_status %in% c("alive", "died"), 1, 0),
         unknown=ifelse(vital_status %in% c("unknown", "moved"), 1, 0)) %>%
  group_by(time_point) %>%
  #finds the number of actual interviews by summing total non-missing interview keys
  summarise(expected=sum(expected), actual=sum(actual), 
            unknown=sum(unknown)) %>%
  ...
  kable("latex", col.names = c("Timepoint", "Expected", "Actual", "Moved/unknown",
                               "Coverage", "N discharged")) %>%
  row_spec(c(0), bold=T)
```
**Moved/unknown** = vital status is moved or unknown.
...
## Mortality rate (Display `r DN`)
```{r, echo=F, warning=F, message=F}
# calculate number of deaths
deaths <- bind_rows(
  # by csps
  pluck(df_list, "followup") %>%
    mutate(died = ifelse(vital_status=="died", 1, 0)) %>%
    group_by(csps_fra) %>%
    summarise(deaths = sum(died, na.rm=T)),
  # overall
  pluck(df_list, "followup") %>%
    mutate(died = ifelse(vital_status=="died", 1, 0)) %>%
    summarise(deaths = sum(died, na.rm=T)) %>%
...
# person-time
bind_rows(
  # calculate person time by csps
  pluck(df_list, "followup") %>%
    group_by(childID) %>%
    # find most recent visit
    mutate(last_fu = max(start_time)) %>%
    ungroup() %>%
    filter(start_time==last_fu) %>%
    left_join(.,
              pluck(df_list, "anthro") %>%
                filter(time_point == "baseline") %>%
                select(childID, enroll_time=start_time),
              by="childID") %>%
    mutate(person_time = as.numeric(difftime(last_fu, enroll_time, units = "days"))) %>%
    # person time by csps
    group_by(csps_fra) %>%
    summarise(person_time = sum(person_time, na.rm=T)),
  
  # calculate person time overall
  pluck(df_list, "followup") %>%
    group_by(childID) %>%
    # find most recent visit
    mutate(last_fu = max(start_time)) %>%
  ...
  mutate(person_time = round(person_time/365.25, 2),
         rate = round(pois.exact(deaths, person_time)[[3]]*1000, 1),
         lower = round(pois.exact(deaths, person_time)[[4]]*1000, 1),
         upper = round(pois.exact(deaths, person_time)[[5]]*1000, 1),
         rate = paste0(rate, " (", lower, " to ", upper, ")")) %>%
  select(csps_fra, deaths, person_time, rate) %>%
  kable("latex", col.names = c("CSPS", "Deaths", "Person-years",
...
```{r, echo=F, message=F, warning=F}
# pull the children that submitted missed visit form
ids <- pluck(df_list, "miss_visit") %>% 
  pull(childID)

# pull the children that submitted discharge form
discharge_ids <- pluck(df_list, "discharge") %>% 
  pull(childID)
...
df_followup %>% 
  ...
  mutate(weeks = weeks(substr(time_point, 6, 6)),
         ...
         missv_form = if_else(childID %in% ids, "Yes", "No"),
         discharge_form = if_else(childID %in% discharge_ids, "Yes", "No")) %>%
  select(-weeks, -follow_update) %>% 
  arrange(csps_fra, -days_late) %>%
  kable("latex", col.names = c("CSPS", "Child ID", "Time point", "Child name",
                               "Enroll date", "Days late", "MVFS?", "DFS?"),
        longtable=T) %>%
  row_spec(c(0), bold=T) %>%
  collapse_rows(1:2, latex_hline = 'custom', custom_latex_hline = 1:2,
                row_group_label_position = 'identity')
```
**MVFS?** = Missed visit form submitted for that child?
**DFS?** = Discharge form submitted for that child?
...
a <- pluck(df_list, "eligibility") %>%
  ...
  select(waz_zscore, muac, time_point, uniqID, signs_of_malnutrition, 
         recovered, csps_fra, id, child_firstname)
...
### Recovered participants (Display `r DN`)
```{r, echo=F, message=F, warning=F}
a %>%
  mutate(childID=substr(uniqID, 0, 7)) %>%
  filter(recovered==1) %>%
  select(csps_fra, id, childID, child_firstname, time_point) %>%
  kable("latex", col.names = c("CSPS", "Interview ID", "Child ID", "Name", "Recovery week"),
        longtable=T) %>%
  row_spec(c(0), bold=T)
```
...
left_join(
  ...
  # bind on the baseline muac for just the children reporting recovery
  bind_rows(., 
            pluck(df_list, "anthro") %>%
  ...
  # create variable that says if recovery is legit or not
  mutate(recovered = if_else(childID %in% rec_ids, "Yes", "No")) %>%
  kable("latex", longtable = T, col.names = c("Child ID", "CSPS", "Name", "Week recovered",
                               "MUAC 1", "MUAC 2", "MUAC 0", "True rec?")) %>%
  row_spec(c(0), bold=T)
```

**MUAC 1** = MUAC at time of recovery. **MUAC 2** = MUAC at week before recovery. **MUAC 0** = MUAC at baseline. **True rec?** = recovered according to study defintion.
...
pluck(df_list, "eligibility") %>%
  ...
  mutate("week_1"=if_else(Sys.Date()-start_time>=7, 1, 0),
         ...
         "week_5"=if_else(Sys.Date()-start_time>=35, 1, 0),
         "week_6"=if_else(Sys.Date()-start_time>=42, 1, 0),
         "week_7"=if_else(Sys.Date()-start_time>=49, 1, 0),
         "week_8"=if_else(Sys.Date()-start_time>=56, 1, 0)) %>%
  gather(key = time_point, value = expected, -start_time, -childID) %>%
  left_join(., 
            pluck(df_list, "anthro") %>% 
              filter(time_point!="baseline" & !duplicated(uniqID)), 
            by=c("childID", "time_point")) %>%
  select(childID, start_time.x, time_point, expected, id, height, weight) %>%
  # create indicators for if child actually had anthro measured or not
  mutate(actual=ifelse(!is.na(weight), 1, 0),
         missing=ifelse(is.na(weight) & !is.na(id), 1, 0)) %>%
  group_by(time_point) %>%
  #finds the number of actual interviews by summing total non-missing interview keys
  summarise(expected=sum(expected), actual=sum(actual), missing=sum(missing)) %>%
  mutate(percnt=percent(actual/expected)) %>%
  arrange(time_point) %>%
  kable("latex", col.names = c("Timepoint", "Expected", "Actual", "Missing anthro", "Percent")) %>%
  row_spec(c(0), bold=T)
```

**Actual** = number of participants with anthro measurement
**Missing anthro** = number of participants missing anthro values on interview
...
pluck(df_list, "followup") %>%
  ...
  select(csps_fra, child_firstname, childID, child_dob, start_time, id) %>%
  kable("latex", col.names = c("CSPS", "Child name", "Child ID", "Child DOB",
                               "Follow-up date", "Interview")) %>%
  row_spec(0, bold=TRUE)
```
...
pluck(df_list, "anthro") %>%
  ...
  kable("latex", col.names = c("CSPS", "Child ID", "Name", "Baseline", "Week 1 change",
                               "Week 2 change", "Week 3 change", "Week 4 change", "Week 5 change",
                               "Week 6 change", "Week 7 change", "Week 8 change"), longtable=T) %>%
  column_spec(1:9, width = "1.7cm") %>%
  row_spec(0, bold=T)
```
...
pluck(df_list, "anthro") %>%
  ...
  kable("latex", col.names = c("CSPS", "Child ID", "Name", "Baseline", "Week 1 change", 
                               "Week 2 change", "Week 3 change", "Week 4 change", "Week 5 change",
                               "Week 6 change", "Week 7 change", "Week 8 change")) %>%
  column_spec(1:9, width = "1.7cm") %>%
  row_spec(0, bold=T)
```
...
pluck(df_list, "rectal") %<>% mutate(childID=paste0(childID, "_", timepoint))
pluck(df_list, "malaria") %<>% mutate(childID=paste0(childID, "_", time_point))
...
map_df(forms, find_duplicate_chids1, df_list) %>%
  ...
  kable("latex", col.names = c("CSPS", "Interview IDs", "Unique ID", "Child name", "Form name")) %>%
  row_spec(0, bold=T) %>%
...
left_join(
  ...
  kable("latex", col.names = c("CSPS", "Child name", "Child ID", "Anthro wt", "Treat wt", 
                               "Difference (kg)")) %>%
  row_spec(0, bold=T)
```