...
con_treated <- pluck(df_list, "treatment") %>% distinct(childID) %>% nrow
...
bind_rows(
  ...
  # table it
  kable("latex", col.names = c("CSPS", "Week 1", "Week 2", "Week 3")) %>%
  row_spec(c(0, overall_rowno), bold=T)
```
...
df_5to7 <- pluck(df_list, "eligibility") %>%
  ...
  gather(key = time_point, value = expected, -enroll_date, -childID) %>%
  left_join(., 
            pluck(df_list, "followup") %>% filter(!duplicated(uniqID)), 
            by=c("childID", "time_point")) %>%
  select(childID, enroll_date, time_point, expected, id)
...
df_followup %>%
  ...
  kable("latex", col.names = c("Timepoint", "Expected", "Actual", "Coverage", "LTFU", "Missing forms", "N discharged")) %>%
  row_spec(c(0), bold=T)
```
...
df_followup %>% 
  ...
  kable("latex", col.names = c("CSPS", "Child ID", "Time point", "Child name", "Enroll date")) %>%
  row_spec(c(0), bold=T) # %>%
...
a <- pluck(df_list, "anthro") %>%
  filter(!duplicated(uniqID) & (time_point!="baseline" & time_point!="week_1")) %>%
  arrange(childID, time_point) %>%
  mutate(rec_whz=ifelse(waz_zscore>= -2 & 
                          lag(waz_zscore, order_by = c(uniqID))>= -2, 
                        1, 0),
         rec_muac=ifelse(muac>= 11.5 & 
                           lag(muac, order_by = c(uniqID))>= 11.5, 
                         1, 0),
         rec_comp=ifelse(grepl("aucun", signs_of_malnutrition), 
                         1, 0),
         recovered=ifelse(
            rec_whz==1 &
            rec_muac==1 &
            rec_comp==1 &
            # must exlcude these false postives bc lag doesn't work for kids with just baseline interview
            time_point!="baseline", 
            1, 0)
         ) %>%
  select(waz_zscore, muac, time_point, uniqID, signs_of_malnutrition, rec_comp, rec_muac, rec_whz, recovered, csps_fra)
...
bind_rows( 
  ...
  row_spec(c(0, overall_rowno), bold=T)
```
**Recovered** = WHZ >= -2 on two consecutive visits AND MUAC >=11.5 and no acute complications or edema over the past 7 days. Since 2 consecutive visits are required to prior to recovery, possible recovery visits only include visits from week 2 and on.
...
rec_date <- left_join(
  ...
  mutate(week_rec = paste0("week_", as.numeric(difftime(start_time, base_time, units = "weeks")))) %>%
...
left_join(
  ...
  kable("latex", col.names = c("Child ID", "CSPS", "Name", "Week recovered", "MUAC 1", "MUAC 2", "MUAC 0", "True recovery")) %>%
  row_spec(c(0), bold=T)
```
...
pluck(df_list, "anthro") %>%
  ...
  kable("latex", col.names = c("CSPS", "Child ID", "Name", "Baseline", "Week 1 change", "Week 2 change", "Week 3 change")) %>%
  row_spec(0, bold=T)
```
