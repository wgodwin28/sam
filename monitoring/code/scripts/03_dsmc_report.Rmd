...
date <- "19Aug2020"
data_cutoff <- as.Date("2020-08-19")
...
commcare-export --output-format csv --output ../../data/raw/temp.zip --project nouna-pilots --query ../../tables/api_metadata.xlsx --username william.godwin@ucsf.edu --password Mordor2018! --since 2020-06-01 --until 2020-08-19
```
...
age <- pluck(df_list, "baseline") %>%
  mutate(age_months = as.integer(age_months),
         age_months = ifelse(Dob_known==1, difftime(data_cutoff, child_dob)/30, age_months)) %>%
...
pluck(df_list, "anthro") %>%
  filter(time_point=="baseline") %>%
  mutate(age_months = ifelse(Dob_known==1, difftime(data_cutoff, child_dob)/30, age_months)) %>%
...
In three participants treatment was not recorded because the dosage of the treatment was not visible on the data collection device.
...
df_5to7 <- pluck(df_list, "eligibility") %>%
  ...
  mutate("week_5"=if_else(data_cutoff-enroll_date>=35, 1, 0),
         "week_6"=if_else(data_cutoff-enroll_date>=42, 1, 0),
         "week_7"=if_else(data_cutoff-enroll_date>=49, 1, 0)) %>%
...
# get the number in the acceptable time window
df_5to7_win <- pluck(df_list, "eligibility") %>%
  filter(enrolled==1 & childID %in% setdiff(childID, discharge_ids)) %>%
  select(childID, enroll_date=start_time) %>%
  mutate("week_5"=if_else(data_cutoff-enroll_date>=35 & 
                                data_cutoff-enroll_date<=37,
                              1, 0),
         "week_6"=if_else(data_cutoff-enroll_date>=42 &
                                data_cutoff-enroll_date<=44,
                              1, 0),
         "week_7"=if_else(data_cutoff-enroll_date>=49 &
                                data_cutoff-enroll_date<=51,
                              1, 0)) %>%
  gather(key = time_point, value = expected_win, -childID, -enroll_date) %>%
  select(-enroll_date)

# join on the expected with the expected window for 5-7 week follow-up
df_5to7 <- left_join(df_5to7, df_5to7_win, by=c("childID", "time_point"))
...
df_followup <- pluck(df_list, "eligibility") %>%
  ...
  mutate("week_1"=if_else(data_cutoff-enroll_date>=7, 1, 0),
         "week_2"=if_else(data_cutoff-enroll_date>=14, 1, 0),
         "week_3"=if_else(data_cutoff-enroll_date>=21, 1, 0),
         "week_4"=if_else(data_cutoff-enroll_date>=28, 1, 0),
         "week_8"=if_else(data_cutoff-enroll_date>=56, 1, 0)) %>%
  gather(key = time_point, value = expected, -enroll_date, -childID, -csps_fra, -child_firstname) %>%
  left_join(., 
            pluck(df_list, "followup") %>% 
              filter(!duplicated(uniqID)) %>% 
...
# get the number in the acceptable time window
df_followup_win <- pluck(df_list, "eligibility") %>%
  filter(enrolled==1) %>%
  select(childID, enroll_date=start_time) %>%
  mutate("week_1"=if_else(data_cutoff-enroll_date>=7 & 
                                data_cutoff-enroll_date<=9,
                              1, 0),
         "week_2"=if_else(data_cutoff-enroll_date>=14 &
                                data_cutoff-enroll_date<=16,
                              1, 0),
         "week_3"=if_else(data_cutoff-enroll_date>=21 &
                                data_cutoff-enroll_date<=23,
                              1, 0),
         "week_4"=if_else(data_cutoff-enroll_date>=28 &
                                data_cutoff-enroll_date<=30,
                              1, 0),
         "week_8"=if_else(data_cutoff-enroll_date>=56 &
                                data_cutoff-enroll_date<=77,
                              1, 0)) %>%
  gather(key = time_point, value = expected_win, -childID, -enroll_date) %>%
  select(-enroll_date)

# join expected with expected in acceptable window
df_followup <- left_join(df_followup, df_followup_win, by=c("childID", "time_point"))

# bind together 1-4,8 weeks with 5-7 weeks
df_followup <- bind_rows(df_followup, df_5to7)
...
df_followup %>%
  ...
  summarise(expected=sum(expected), actual=sum(actual), 
            unknown=sum(unknown), window=sum(expected_win)) %>%
  ...
  kable("latex", col.names = c("Timepoint", "Expected", "Actual", "Moved/unknown",
                               "In window", "Coverage", "N discharged")) %>%
  row_spec(c(0), bold=T)
```
...
**In window** = number within the acceptable follow-up window.
...
## Severe Adverse Events (Display `r DN`)
```{r, echo=F}
# read in data
readxl::read_xlsx("~/Box/SAM Pilot/SAE/SAE master file SAM.xlsx") %>%
  kable("latex") %>%
...
There was one additional participant who was discharged due to death but not included since it was reported after the data cutoff date. **Transferred** = participant admitted for inpatient treatment.
